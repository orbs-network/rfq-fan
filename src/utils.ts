...
import { TokenData, contract, eqIgnoreCase, erc20FromData, isNativeAddress, maxUint256, web3 } from "@defi.org/web3-candies";
...
export function getSignature(config: Config, serializedOrder: string, signature: string) {
...
export async function getTransactionDetails(web3: Web3, txHash: string) {
    try {
        ...
        const transferEvents = receipt.logs.filter((log) => log.topics[0] === ERC20_TRANSFER_EVENT_SIGNATURE);
...
export function dumpFetchAsCurl(url: string, options: any) {
    ...
    return logger.verbose(`curl -X ${method} ${headerString} ${bodyString} '${url}'`);
}