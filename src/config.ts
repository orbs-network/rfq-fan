...
const ChainConfigs = {
  ...
  blast: {
    ...
    logoUrl:
      "https://cdn.routescan.io/_next/image?url=https%3A%2F%2Fcms-cdn.avascan.com%2Fcms2%2Fblast.dead36673539.png&w=256&q=100",
...
export function withConfig(e: EventUrl): CommonConfig {
  ...
  const chainIdStr = e.queryStringParameters?.chainid || e.queryStringParameters?.chainId;
  const chainId = chainIdStr ? parseInt(chainIdStr) : 0
  if (!chainId) {
    throw new Error(`a valid chainId is required ${e}`);
  }
  ...
  let networkUrlBackup =
    secrets[urlKeyBackup] || (process.env as any)[urlKeyBackup];
