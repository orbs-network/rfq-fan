...
export async function getTokenPrice(
  ...
  if (
    ...
    (tokenAddress === "0x1e4a5963abfd975d8c9021ce480b42188849d41d" ||
      tokenAddress === "0x37eaa0ef3549a5bb7d431be78a3d99bd360d19e5")
  ) {
    return {
      priceUsd: 1,
      priceNative: 1,
  ...
  if (price.priceUsd) {
    redis.setX(
      _price2Key(c.chainId, tokenAddress),
...
export async function getTokenPriceLegacy(
  c: CommonConfig,
  tokenAddress: string,
): Promise<TokenPrice> {
  const chainId = c.chainId;
  ...
  if (price.priceUsd > 0) {
    ...
    redis.setX(
      _priceKey(chainId, tokenAddress),
      JSON.stringify(price),
      PRICE_REDIS_TTL,
    );
  } else {
...
async function fetchPriceDexScreener(
  ...
  try {
    ...
    if (!data.pairs[0]) {
      let paraPrice = await fetchPriceParaswap(
        chainId,
        tokenAddress,
        data.decimals,
      );
    ...
    logger.warn(
      `⛔️ ⛔️ ⛔️ fetchPrice: ${tokenAddress} failed ${e} fallback to paraswap cid:${chainId}`,
    );
...
export async function fetchLLMAPrice(token: string, chainId: number | string) {
  ...
  try {
    ...
    logger.warn(
      `Price-Oracle ✨ : fetchLLMAPrice: ${chainId}:${token} failed`,
      error,
    );
    return nullPrice;
  }
...
export async function fetchPriceParaswap(
  ...
  const url = `https://apiv5.paraswap.io/prices/?srcToken=${inToken}&destToken=${
    n.wToken.address
  }&amount=${BN(
    `1e${inTokenDecimals}`,
  ).toString()}&srcDecimals=${inTokenDecimals}&destDecimals=${
    n.wToken.decimals
