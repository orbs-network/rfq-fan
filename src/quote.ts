...
export async function quote(
  ...
  const [q, outToken] = await Promise.all([
    ...
    isNativeAddress(rfq.outToken)
      ? Promise.resolve(config.native)
  ...
  let simulatedOutAmount = BN(
    q.result[0]?.simulatedSwapResult?.outAmount || "0",
  );
  ...
  if (reduceGasCost) {
    ...
    logger.warn(
      `[${rfq.sessionId}] quote::gasUnits ${gasUnits.toFixed(
        0,
    ...
    if (solver.name === "rango") {
      ...
      gasCostNative = gasUnits.plus(
        bn(gas.maxFeePerGas).multipliedBy(config.baseGasCost),
      );
    }

    if (solver.name === "orion") {
      //@ts-ignore
      gasCostNative = BN(config.exchanges["orion"].swapGasUnits).multipliedBy(
        config.baseGasCost,
      );
    }
    ...
    if (solver.name === "magpie") {
      //@ts-ignore
      gasCostNative = BN(config.exchanges["magpie"].swapGasUnits).multipliedBy(
        config.baseGasCost,
      );
    }
    ...
    logger.info(
      `[${rfq.sessionId}] [${solver}] ⛽️ gasPriceInNative: ${gasCostNative
        .dividedBy(1e18)
        .toFixed(4)} gasUnits: ${gasUnits.toFixed(0)}`,
    );
    if (config.fixedGasCost) {
      gasCostNative = bn(config.fixedGasCost);
    }
    gasCostOutputToken = await gasToOutputToken(
      config,
      gasCostNative,
      rfq.outToken,
      solver,
    ...
    logger.debug(
      `[${rfq.sessionId}] [${solver}]⛽️ gasCostOutputToken:[${rfq.sessionId}] gasCostOutputToken: ${gasCostOutputToken}`,
    );
  ...
  const data = q.result[0].fillData
    ? q.result[0].fillData
    : { to: "", data: "" };
...
export async function quoteLite(
  config: CommonConfig,
  rfq: RFQ,
  solver: ISolver,
  pathFinderParams = null,
...
async function callExchangeQuote(
  config: CommonConfig,
  rfq: RFQ & { slippage?: number },
  solver: ISolver,
  quoteWithData: boolean,
  pathFinderParams?: any,
) {
  const now = Date.now();
  ...
  if (
    (!quoteWithData && config.useLiteQuote) ||
    (solver.solverType === "Offchain" && !quoteWithData)
  ) {
  ...
  if (
    ...
    logger.warn(
      `[${rfq.sessionId}] callExchangeQuote::error [${solver.name}] => ${data.error} ${data.errorMessage} message:${data?.result[0].errorMessage}`,
    );
...
async function gasToOutputToken(
  config: CommonConfig,
  gasCostNative: BN,
  outToken: string,
  solver: ISolver,
  ...
  logger.debug(
    `gasToOutputToken[${sessionId}] <${solver}> ==> outToken: ${outToken} exchange: ${solver} user: ${user} gasCostNative: ${gasCostNative
      .dividedBy("1e18")
      .toFixed(0)} `,
  );
  ...
  try {
    if (
      config.wToken.address.toLowerCase() == outToken.toLowerCase() ||
    ...
    return gasToOutputTokenFallback({
      config,
...
async function gasToOutputTokenFallback(params: {
  config: CommonConfig;
  gasCost: BN;
  outToken: string;
  solver: ISolver;
  ...
  logger.warn(
    `⚠️ gasToOutputTokenFallback:[${params.sessionId}]:[${params.solver}]`,
  );
  ...
  if (nativeToOutputQuote?.isError) {
    logger.warn(
      `gasToOutputToken::nativeToOutputQuote:error:[${sessionId}]:[${solver}] ${JSON.stringify(
        nativeToOutputQuote,
      )}`,
    );
    return bn(0);
  ...
  logger.debug(
    `gasToOutputToken::nativeToOutputQuote:[${sessionId}]: ${JSON.stringify(
      nativeToOutputQuote,
    )}`,
  );
  logger.debug(
    `nativeToOutputQuote.outAmount:[${sessionId}]:[${solver}] ${outAmount}`,
  );
...
export async function getScore(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const fails = Number(
        await redisWrapper.get(solverFailuresKey(chainId, solver)),
      );
      const success = Number(
        await redisWrapper.get(solverSuccessKey(chainId, solver)),
      );
